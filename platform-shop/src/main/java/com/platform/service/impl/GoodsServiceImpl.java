...
import com.platform.entity.GoodsSpecifcationEniity;
...
public class GoodsServiceImpl implements GoodsService {
    ...
    @Override
    public List<GoodsSpecifcationEniity> getSpecifications(Integer id) {
	//首先根据商品ID查询出对应的规格信息
	List<SpecificationEntity> Specification = goodsDao.getSpecifications(id);
	List<GoodsSpecifcationEniity> goodsSpecifcationEniities = new ArrayList<>();
       for(SpecificationEntity specificationEntity:Specification) {
	  GoodsSpecifcationEniity eniity=new GoodsSpecifcationEniity();
	    // 设置标规格得名字
	    eniity.setGoodsSpecifcationName(specificationEntity.getName());
	  Map<String, Object> pamas=new HashMap<>();
	  pamas.put("goodsId", id);
	  pamas.put("specificationId", specificationEntity.getId());
	    // 查询具体得规格信息
	 List<GoodsSpecificationEntity>  cc=goodsSpecificationDao.queryList(pamas);
	 eniity.setGoodsSpecifcationInfo(cc);
	 goodsSpecifcationEniities.add(eniity);
       }
       return goodsSpecifcationEniities;
