...
Page({
  ...
  bindinputMobile(event) {
    ...
    if (event.detail.value.length == 11 && !str.test(event.detail.value)) {
  ...
  saveAddress(){
    ...
    let address = this.data.address;
    if (address.userName == '') {
      util.showNoIconErrorToast('请输入姓名');
      return false;
    }
    ...
    if (address.telNumber.length < 11) {
      util.showNoIconErrorToast('请输入正确的手机号码');
      return false;
    }

    if (address.telNumber.length == 11 && !str.test(address.telNumber)) {
      util.showNoIconErrorToast('请输入正确的手机号码');
      return;
    }

    if (address.district_id == 0) {
      util.showNoIconErrorToast('请输入省市区');
      return false;
    }

    if (address.detailInfo == '') {
      util.showNoIconErrorToast('请输入详细地址');
      return false;
    }
