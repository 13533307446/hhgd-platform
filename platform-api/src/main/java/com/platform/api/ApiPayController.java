...
public class ApiPayController extends ApiBaseAction {
    ...
    public Object payPrepay(@LoginUser UserVo loginUser, Integer orderId) {
		...
		if (loginUser.getUserId() != orderInfo.getUser_id()) {
			return toResponsObject(400, "订单号错误", "");
		}
    ...
    public void notify(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			logger.info("微信支付返回参数" + reponseXml);
		...
		orderInfo.setTransactionNo(result.getTransaction_id());
