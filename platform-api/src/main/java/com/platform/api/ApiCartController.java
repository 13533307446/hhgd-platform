...
public class ApiCartController extends ApiBaseAction {
    ...
    public Object add(@LoginUser UserVo loginUser) {
        ...
        String goodsSepcifitionPicUrl = jsonParam.getString("goodsPicUrl");
        ...
        if (null == cartInfo) {
			...
			cartInfo.setList_pic_url(goodsSepcifitionPicUrl);
    ...
    public Object checkout(@LoginUser UserVo loginUser, Integer couponId) {
        ...
        AddressVo  voAddress  =  new  AddressVo();
        if(addressEntities==null  || addressEntities.size()==0  )  {

            voAddress.setId(-1L);
            resultObj.put("checkedAddress", voAddress);
        } else  {
			//设置第一次
